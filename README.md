...
В конце тестирования выводится значение ("search perforamnce test dummy variable"), являющееся суммой всех результатов запросов и равное `36625556600`. Данное число не должно меняться при использовании одной и той же пары множества входных слов и множества слов для поиска.
### Профилирование
...
make perf_report
```
 - Показывает, какие инструкции ассемблера занимают больше всего времени:
```
make perf_annotate
```
...
1. Для уменьшения влияния сторонних факторов тестирование производится при единственной открытой программе (терминале) и выбранном режиме батареи `performance`. (см. ниже примечание про частоту процессора)

2. Выполняется команда (`make perf_report`), которая возвращает количество сделанных сэмплов, которые совершаются с заявленной частотой 4000Гц.

3. Пункт `2.` выполняется ~20 раз подряд, из которых используются последние 10 полученных значений сэмплов.

4. Указанные значения сэмплов записываются в файл `opt_smpls/opt_smpls_N`, где `N` - номер оптимизации, каждое на своей строчке.

5. С помощью скрипта вычисляется среднее значение, его абсолютная и относительная ошибка: `python3 py_scripts/abs_error.py opt_smpls/opt_smpls_N`

6. Данное значение сравнивается с предыдущим и делается вывод об эффективности оптимизации.

**Примечание о частоте процессора:** приведенная методика была бы значительно точнее при залочивании частоты процессора, но простой способ этого добиться не был найден в короткие сроки.

**О вычисляемой абсолютной ошибке:** Это только случайный компонент ошибки, фактически являющейся оценкой снизу, но в данной работе им можно оценивать саму абсолютную ошибку среднего количества сэмплов. Эффективность оптимизации считается в сэмплах, но оценочно перевести в секунды можно, поделив на частоту совершения сэмплов (4000Гц)
